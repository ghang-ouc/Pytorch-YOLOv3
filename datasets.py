...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16, augment=False, hyp=None, rect=False,
                 cache_images=False, pad=0.0):
        try:
            path = str(Path(path))  # os-agnostic
            ...
            #  os.path.splitext(path) 分离文件名和扩展名 默认返回(fname,fextension)元组
            self.img_files = [x.replace('/', os.sep) for x in f if os.path.splitext(x)[-1].lower() in img_formats]
        except:
            raise Exception('Error loading data from %s.' % path)
        n = len(self.img_files)  # 图片数量
        assert n > 0, 'No images found in %s.' % path  # 图片数量要大于0
        bi = np.floor(np.arange(n) / batch_size).astype(np.int)  # np.floor 向下取整
        nb = bi[-1] + 1  # 根据batch_size一个epoch包含几个batch

        self.n = n  # 图片数量
        self.batch = bi  # 图片的batch序号
        self.img_size = img_size
        ...
        try:
            with open(sp, 'r') as f:  # read existing shapefile
                s = [x.split() for x in f.read().splitlines()]  # splitlines() 按照行界符('\r', '\r\n', \n'等)分隔
        ...
        for i, file in enumerate(pbar):
            ...
            pbar.desc = 'Caching labels %s (%g found, %g missing, %g empty, %g duplicate, for %g images)' % (
                s, nf, nm, ne, nd, n)
        assert nf > 0 or n == 20288, 'No labels found in %s.' % os.path.dirname(file) + os.sep
